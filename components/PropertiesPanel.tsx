...
import { Segment, Transition, TransitionType, VideoAnalysis, SubtitleStyle } from '../types';
...
const FONTS = ['Arial', 'Verdana', 'Helvetica', 'Times New Roman', 'Courier New', 'Georgia', 'Impact'];
...
const PropertiesPanel: React.FC<PropertiesPanelProps> = ({ 
  ...
  if (!selectedSegment && !selectedTransition && !selectedDialogue) {
    return (
      <div className="h-full bg-[#1e1e1e] border-l border-[#333] p-4 flex items-center justify-center text-gray-500 text-xs text-center">
        Select a clip, transition, or dialogue bubble to view properties
      </div>
    );
  }
    ...
    <div className="h-full bg-[#1e1e1e] border-l border-[#333] flex flex-col font-sans">
      <div className="p-3 border-b border-[#333] bg-[#252525]">
        <h2 className="text-xs font-bold uppercase tracking-widest text-gray-300">
          {selectedTransition ? 'Transition Properties' : selectedDialogue ? 'Subtitle Properties' : 'Clip Properties'}
        </h2>
      </div>
